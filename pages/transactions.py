...
def perform_inference(transactions_df, rf_model, lof_model):
    ...
    X_rf = transactions_df.drop(columns=['fraud_bool', 'LOF Status', 'ref_id'], errors='ignore')
    ...
    # Apply LOF on transactions classified as non-fraud by RF, including 'ref_id' in non_fraud_df
    ...
    if not non_fraud_df.empty:
        ...
        transactions_df.update(non_fraud_df[['LOF Status', 'lof_scores']])
